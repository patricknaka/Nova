...
-- 15/12/2015 ID MMF - Diversas colunas foram tiradas do select e inseridas em jois. A query foi equalizada com a nike (sem id)
--*************************************************************************************************************************************************************
SELECT
  ...
  CASE WHEN znmcs095.t$sige$c IS NOT NULL THEN  --Pedido SIGE
	...
	cisli943_icms.t$amnt$l	VL_ICMS,						--MMF
...
--	ELSE CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(znsls400org.t$dtin$c, 'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')
  ...
  END                             	NR_SERIE_NF_REMESSA,	
	cisli943_pis.t$amnt$l		VL_PIS,					--MMF
	cisli943_cofins.t$amnt$l	VL_COFINS,				--MMF
  ...
  CASE WHEN znmcs095.t$sige$c IS NOT NULL THEN  --Pedido SIGE
    ...
    CASE WHEN cisli940org.t$fdty$l = 15 THEN    --Remessa para presente
      cisli940rem_fat.t$fire$l    --Fatura para presente
    ELSE
--      cisli940org.t$fire$l 												-- MMF.o
      cisli941dev.t$refr$l 											-- MMF.n
    END 
