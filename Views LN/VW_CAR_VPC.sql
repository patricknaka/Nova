SELECT DISTINCT
  ...
  WHEN LENGTH(regexp_replace(znrec007.t$fovn$c, '[^0-9]', ''))<11
    THEN '00000000000000'
  ELSE regexp_replace(znrec007.t$fovn$c, '[^0-9]', '') 
  END                                             NR_CNPJ_CPF,
  ...
  WHEN LENGTH(regexp_replace(znrec007.t$fovf$c, '[^0-9]', ''))<11
    THEN '00000000000000'
  ELSE regexp_replace(znrec007.t$fovf$c, '[^0-9]', '') 
  END                                             NR_CNPJ_FILIAL,
  CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(znrec007.t$datf$c, 
    'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')
      AT time zone 'America/Sao_Paulo') AS DATE)  DT_INICIAL,
  CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(znrec007.t$datt$c, 
    'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')
      AT time zone 'America/Sao_Paulo') AS DATE)  DT_FINAL,
  CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(znrec007.t$datp$c, 
    'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')
      AT time zone 'America/Sao_Paulo') AS DATE)  DT_VENCIMENTO,
  ...
  CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(znrec007.t$udat$c, 
    'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')
...
WHERE znrec007.t$ttyp$c = tfacr200.t$ttyp 
