SELECT 
  ...
  1                                               CD_CIA,
	...
	TO_CHAR(brnfe940.t$docn$l)                       NR_NF_REFERENCIA,																
...
--e quando importamos os dados, eles estão vindo como 01/01/4712 e precisa vir como nulo
case when to_char(brnfe940.t$idat$l, 'yyyy') = 4712				-- MMF.477
...
--		AT time zone 'America/Sao_Paulo') AS DATE)DT_SAIDA_NF,
  ...
  CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(brnfe940.t$fcdt$c, 
	...
	(select CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(min(ttdpur451201.t$trdt), 
    'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')
  ...
  CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(
	...
	nvl((select sum(r.T$CCHR$L) 
    FROM baandb.ttdrec941201 r
		where r.t$fire$l = brnfe940.t$frec$l),0)      VL_DESPESA_ADUANEIRA,
	CASE WHEN tdpur400.t$rfdt$l = 37 
    THEN brnfe940.t$addc$l ELSE 0 END             VL_ADICIONAL_IMPORTACAO,
	CASE WHEN tdpur400.t$rfdt$l = 37 THEN 
		(SELECT SUM(i.t$amnt$l) 
      FROM baandb.tbrnfe942201 i
			WHERE i.t$fire$l = brnfe940.t$fire$l 
      AND i.t$brty$l = 5) ELSE 0 END              VL_PIS_IMPORTACAO,
	CASE WHEN tdpur400.t$rfdt$l = 37 THEN
		(SELECT SUM(i.t$amnt$l) 
      FROM baandb.tbrnfe942201 i
			WHERE i.t$fire$l = brnfe940.t$fire$l 
      AND i.t$brty$l = 6) ELSE 0 END              VL_COFINS_IMPORTACAO,	
	CASE WHEN nvl((select a.t$cdec 
        from baandb.ttdpur400201 a
...
where brnfe940.t$fire$l = 'PR0002688'