CREATE OR REPLACE VIEW VW_SKU AS
SELECT  
   ...
   CASE WHEN tcmcs023.t$tpit$c=2 
     THEN 10
   WHEN tcmcs023.t$tpit$c=3 
     THEN 20
   ELSE tcibd001.t$espe$c END                                                 CD_ITEM_CONTROLE,
   ...
   CASE WHEN tcibd001.t$csig in ('SUS','CAN')
     THEN 1
   ELSE 2 END                                                                 IN_ITEM_SUSPENSO,

   CASE WHEN tcibd001.t$csig='SNG' 
     THEN 1
   ELSE 2 END                                                                 IN_ITEM_SINERGIA,

   CASE WHEN tdisa001.t$dtla$c > TO_DATE('10-JAN-1990','DD-MON-YYYY') 
     THEN tdisa001.t$dtla$c
...
LEFT JOIN baandb.ttdisa001301 tdisa001 
...
LEFT JOIN baandb.ttccom100301 tccom100f 
       ON tccom100f.T$BPID=tcmcs060.T$OTBP
