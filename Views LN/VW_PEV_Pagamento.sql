...
select
  ...
  znsls402.t$nupa$c  NR_PARCELAS,
	cast((sls401q.VL_PGTO_ENTR/sls401p.VL_PGTO_PED)*znsls402.t$vlmr$c as numeric(12,2)) 
  AS VL_PAGAMENTO,
  znsls402.t$stat$c  CD_STATUS_PAGAMENTO,
	CASE WHEN (znsls402.t$idmp$c = 4 and znsls400.t$idli$c!=0) THEN 1 ELSE 2 END 
  AS IN_VALE_LISTA_CASAMENTO,
	CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(znsls400.t$dtem$c, 'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')
    AT time zone sessiontimezone) AS DATE) 
  AS DT_EMISSAO_PEDIDO,
  ...
  cast((sls401q.VL_PGTO_ENTR/sls401p.VL_PGTO_PED)*znsls402.t$vlja$c as numeric(12,2))  VL_JUROS_ADMINISTRADORA,
    ...
    (select CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(min(a.t$trdt), 'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')
      ...
      where a.t$orno=tdsls400.t$orno) 
  AS DT_APROVACAO_PAGAMENTO_ERP,
