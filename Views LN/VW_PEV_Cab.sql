SELECT  DISTINCT
         CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(greatest(tdsls400.t$rcd_utc, znsls410.dtoc), 'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')
        ...
        TO_CHAR(znsls401.entr)                      NR_ENTREGA,                                      --#FAF.046.n
        ...
        znsls401.opfc                               CD_NATUREZA_OPERACAO,
        ' '                                         SQ_NATUREZA_OPERACAO,        -- *** NAO EXISTE NA PREVISAO DE IMPOSTOS
        tdsls400.t$ccur                             CD_MOEDA,
        tdsls400.t$hdst                             CD_SITUACAO_PEDIDO,
        CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(tdsls450.t$trdt, 'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')
        AT time zone 'America/Sao_Paulo') AS DATE)  DT_SITUACAO_PEDIDO,
        ...
        znsls410.poco                               CD_STATUS,
        znsls410.dtoc                               DT_STATUS_PEDIDO,
...
FROM    baandb.ttdsls400201 tdsls400
 ...
 LEFT JOIN (select  c245.T$SLSO, 
                    c940.T$DOCN$L NOTA, 
                    c940.t$seri$l SERIE             --#FAF.006.sn
        ...
        ON consold.T$SLSO = tdsls400.t$orno          --#FAF.006.en
 ...
 INNER JOIN (SELECT
                  ...
                  a.t$sqpd$c       sqpd,
                  a.t$entr$c       entr,
                  case when a.t$qtve$c < 0 then 2 else 1 end cancela,
                  a.t$sedt$c       seq_pedido_cancel,
                  a.t$endt$c       entrega_cancel,
                  max(a.t$pztr$c)  pztr,
                  max(a.t$pzcd$c)  pzcd,
                  max(a.t$pcga$c)  pcga,                                  --#FAF.177.n
                  CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(max(a.t$dtep$c), 'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')
                  AT time zone 'America/Sao_Paulo') AS DATE) dtep,
                  max(a.t$itpe$c)  itpe,
                  max(a.t$mgrt$c)  mgrt,                                  --#FAF.177.n
                  max(tcibd001.t$tptr$c)  tptr,                           --#FAF.177.n
            ...
            GROUP BY
                  ...
                  a.t$entr$c,
                  a.t$orno$c,
                  case when a.t$qtve$c < 0 then 2 else 1 end,
                  a.t$sedt$c,
                  a.t$endt$c,
                  brmcs941.t$opfc$l,
                  a.t$idor$c) znsls401
...
-- INNER JOIN baandb.tznsls400201 znsls400    --ALTERADO PARA MELHORIA DE PERFORMANCE
 ...
 INNER JOIN baandb.ttdsls094201 tdsls094                     --#FAF.006.n
         ON tdsls094.t$sotp = tdsls400.t$sotp                --#FAF.006.n
 ...
 LEFT JOIN ( select a.t$ncia$c,
           ...
           group by a.t$ncia$c,
                    ...
                    a.t$pecl$c,
                    a.t$sqpd$c ) znsls410
        ON znsls410.t$ncia$c = znsls401.ncia
       AND znsls410.t$uneg$c = znsls401.uneg
       AND znsls410.t$pecl$c = znsls401.pecl
       AND znsls410.t$sqpd$c = znsls401.sqpd

 LEFT JOIN (select  a.t$orno,
                    max(a.t$trdt)  t$trdt
            from    baandb.ttdsls450201 a
            group by a.t$orno ) tdsls450
        ON tdsls450.t$orno = tdsls400.t$orno    --DT_SITUACAO_PEDIDO
