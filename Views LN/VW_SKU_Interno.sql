SELECT  
  ...
  WHEN LENGTH(regexp_replace(tccom130f.t$fovn$l, '[^0-9]', ''))<11
    THEN '00000000000000'
  ELSE regexp_replace(tccom130f.t$fovn$l, '[^0-9]', '') 
  END                                 NR_CNPJ_FABRICANTE,
  ...
  CASE 
    WHEN tcibd001.t$csig='SUS' 
      THEN 1
  ELSE 2
  END                                 IN_ITEM_SUSPENSO,
  CASE 
    WHEN tdisa001.t$dtla$c>TO_DATE('10-JAN-1990','DD-MON-YYYY') 
      THEN tdisa001.t$dtla$c
  ELSE NULL 
  END                                 DT_LANCAMENTO_PREVENDA,
...
FROM  baandb.ttcibd001201 tcibd001
  ...
  LEFT JOIN baandb.ttdisa001201 tdisa001 
  ...
  LEFT JOIN baandb.ttccom130201 tccom130f 
