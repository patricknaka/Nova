...
where   znfmd001.t$fili$c in (:PLANTA)                                  
and     trunc(cast((FROM_TZ(TO_TIMESTAMP(TO_CHAR(ORDERSTATUSHISTORY.LAST_DATE, 
                           'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')
                             AT time zone 'America/Sao_Paulo') as date))
            between :DATA_ULTIMO_EVENTO_DE and :DATA_ULTIMO_EVENTO_ATE
and     tdsls400.t$cdec in (:TIPO_ENTREGA)
and     ORDERSTATUSSETUP.CODE in (:TIPO_EVENTO)
order by
        znsls401.t$pecl$c, tisfc001.t$pdno
