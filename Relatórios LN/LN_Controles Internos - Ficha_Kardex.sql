SELECT
  ...
  CASE WHEN sum(whina112.t$qstk) <> 0.00 
    THEN 'E' WHEN sum(whina114.t$qstk) <> 0.00 
    THEN 'S' END                CODE_ES,
  CASE WHEN whinr110.t$kost = 5 
    THEN cisli940.t$docn$l 
    ELSE tdrec940.t$docn$l END  DOC_FISCAL
...
( SELECT d.t$cnst CODE_ORDEM, l.t$desc DESC_ORDEM
    ...
    WHERE whinr110.t$kost > 0
      ...
      AND  CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(whinr110.t$trdt, 'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')
