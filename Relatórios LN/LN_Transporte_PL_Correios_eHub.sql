    SELECT znfmd630.t$etiq$c                                   ETIQUETA,
           znfmd065.t$cser$c                                   COD_SERVICO,
...
WHERE znfmd630.T$CFRW$C = :Trasmportadora
  AND Trunc(CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(znfmd640.t$date$c, 
      ...
      Between :DataLiquidacaoDe
          And :DataLiquidacaoAte

GROUP BY znfmd630.t$etiq$c,
         znfmd065.t$cser$c,
         tccom130.t$pstc,
         znfmd630.t$wght$c,
         znfmd630.t$ncar$c,
...
"        AND znfmd640.t$etiq$c = znfmd630.t$etiq$c  " &
"  " &
" WHERE znfmd630.T$CFRW$C = :Transportadora  " &
"   AND Trunc(CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(znfmd640.t$date$c,  " &
"               'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')  " &
"                 AT time zone 'America/Sao_Paulo') AS DATE))  " &
"       Between :DataLiquidacaoDe  " &
