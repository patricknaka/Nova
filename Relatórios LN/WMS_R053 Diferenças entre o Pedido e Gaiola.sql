...
FROM
   ...
   (select 		a.t$ncia$c,
	...
	INNER JOIN	BAANDB.TTCIBD001301 TCIBD001		ON		TCIBD001.T$ITEM   = ZNSLS401.T$ITEM$C
	INNER JOIN 	WMWHSE5.ORDERS@DL_LN_WMS	OWMS	ON		OWMS.REFERENCEDOCUMENT = ZNSLS004.T$ORNO$C
	INNER JOIN	ENTERPRISE.CODELKUP@DL_LN_WMS CL ON		UPPER(CL.UDF1) = OWMS.WHSEID
	...
	INNER JOIN	(	select	f.t$orno$c,
					...
					inner join 	baandb.tznfmd640301	o	on	o.t$fili$c = f.t$fili$c
														and	o.t$etiq$c = f.t$etiq$c
					group by f.t$orno$c,
							 f.t$ncar$c,
							 f.t$fili$c)	ULT_OCR	ON		ULT_OCR.T$ORNO$C = ZNSLS004.T$ORNO$C
	INNER JOIN	BAANDB.TZNFMD030301 ZNFMD030		ON		ZNFMD030.T$OCIN$C= ULT_OCR.T$COCI$C
	LEFT JOIN (select 	nvl(c.t$vpla$c,nvl(c.t$vplc$c, c.t$vplt$c)) placa,
						l.t$fili$c,
						l.t$ncar$c
				from		baandb.tznfmd170301 c
				inner join	baandb.tznfmd171301 l on  l.t$fili$c = c.t$fili$c
												  and l.t$nent$c = c.t$nent$c
												  and l.t$cfrw$c = c.t$cfrw$c
				group by nvl(c.t$vpla$c,nvl(c.t$vplc$c, c.t$vplt$c)),
                         l.t$fili$c,
                         l.t$ncar$c)	CCPLACA		ON		CCPLACA.t$fili$c = ULT_OCR.T$FILI$C
