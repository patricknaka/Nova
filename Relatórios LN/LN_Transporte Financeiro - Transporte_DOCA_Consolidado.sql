SELECT  
        ...
        CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(
          CASE WHEN (a.INVOICESTATUS = '2' and a.status >  = '55') or a.status = '100' THEN
            a.editdate
...
,
         ...
         WHERE UPPER(schm.UDF1) = a.whseid
           ...
           AND CASE WHEN a.FISCALDECISION like 'CANCELADO%' then 1
               ELSE 0 END = 0
...
select Q1.*
  from ( SELECT 
		...
		LEFT JOIN WMWHSE1.OrderDetailXvas OX
			ON 	OX.ORDERKEY = od.ORDERKEY
			AND	OX.ORDERLINENUBER = = od.ORDERLINENUBER 
            AND OX.UDF1 = 'SHIPPINGID'
