SELECT 
		...
		WSOR.ADDDATE							DT_REGISTRO,
		WOSS.DESCRIPTION						EVENTO,
		WSOR.EDITDATE							DT_EVENTO,
		...
		WSOR.LANE								NFSR,
		NVL(TCMCS966.T$DSCA$L,TCMCS966x.T$DSCA$L)						DESCR_TP_DOC_FIS,
		WHINH010.T$DSCA							NOME_OPER,
		...
		WHWMD400.T$HGHT
		* WHWMD400.T$WDTH
		* WHWMD400.T$DPTH						M3_UNIT,
		WHINH220.T$QORO 						QT_PED,
		WHINH220.T$QORO 						QT_CAN,					--	Será sempre o mesmo que a quantidade do pedido pois no Ln não temos entrega parcial
		NVL(CISLI941.T$DQUA$L,0)				QT_FAT,
		ABS(CISLI941.T$PRIC$L)					VL_NF_UNIT,
		ABS(CISLI941.T$AMNT$L)					VL_NF_TOTAL,
		WSOR.EDITWHO							USUARIO_CANCEL,
		DIVS.DESCRIPTION						SITUACAO_FAT
...
LEFT JOIN (SELECT A.T$FIRE$L,
           ...
           WHERE A.T$DATG$L = (SELECT MIN(B.T$DATG$L) FROM BAANDB.TCISLI940301 B
...
--          and wsor.invoicenumber!=0
         --AND WHINH200.T$ORNO='OA0000696'