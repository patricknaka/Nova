...
GROUP BY WSOR.WHSEID, wmsCODE.UDF2, WHINH200.T$ORNO, WSOR.ORDERKEY, WSOR.ADDDATE, 
         WOSS.DESCRIPTION, WSOR.EDITDATE, cisli940.t$DATE$L, WSOR.INVOICENUMBER,
         WSOR.LANE,NVL(TCMCS966.T$DSCA$L, TCMCS966x.T$DSCA$L), WHINH010.T$DSCA,
         TRIM(WHINH220.T$ITEM), TCIBD001.T$DSCA, TCMCS023.T$DSCA, LNCF.T$FOVN$L,
         LNFB.T$DSCA, LNCC.T$FOVN$L, LNCC.T$NAMA, WHWMD400.T$HGHT * WHWMD400.T$WDTH *
         WHWMD400.T$DPTH, WSOR.EDITWHO, DIVS.DESCRIPTION
...
"  WHWMD400.T$DPTH        M3_UNITARIO,  " &
"  SUM(WHINH220.T$QORO)      QTD_PEDIDO,  " &
"  SUM(WHINH220.T$QORO)      QTD_CANCELADA,  " &
"  NVL(SUM(CISLI941.T$DQUA$L),0)QTD_FATURADA,  " &
"  ABS(MAX(CISLI941.T$PRIC$L))  VALOR_NF_UNITARIO,  " &
"  ABS(SUM(CISLI941.T$AMNT$L))  VALOR_NF_TOTAL,  " &
"  WSOR.EDITWHO           USUARIO_CANCELAMENTO,  " &
