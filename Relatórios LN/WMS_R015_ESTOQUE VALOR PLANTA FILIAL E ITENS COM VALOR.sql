SELECT
  ...
  CASE WHEN sku.ACTIVE = '2' 
       ...
       ELSE   'Ativo' 
   END                                 SITUACAO_ITEM,
  CASE WHEN departSector.depart_name IS NULL THEN
    LN_DEPTO.t$dsca
  ELSE TO_CHAR(departSector.depart_name) END    DEPTO,
  CASE WHEN departSector.sector_name IS NULL THEN
        LN_SETOR.t$dsca$c
  ELSE
        TO_CHAR(departSector.sector_name) END   SETOR, 
  Trim(pz.DESCR)                       CLAL,
  ll.loc                               LOCA,
...
INNER JOIN WMWHSE5.loc 
 ...
 LEFT JOIN ( select clkp.code          COD_TIPO, 
  ...
  LEFT JOIN BAANDB.TTCMCS023301@PLN01 LN_DEPTO
         ON LN_DEPTO.T$CITG = LN_ITM.T$CITG
         
  LEFT JOIN BAANDB.TZNMCS030301@PLN01 LN_SETOR
