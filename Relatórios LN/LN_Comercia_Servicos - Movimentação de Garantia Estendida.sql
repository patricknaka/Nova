SELECT ZNCOM005.T$UNEG$C                               UNEG,
       ...
       CASE WHEN ZNSLS401_P.T$PECL$C IS NULL THEN
            ZNSLS004.T$PECL$C
       ELSE ZNSLS401_P.T$PECL$C END                    NR_PEDIDO_PRODUTO,
       ...
       CASE WHEN ZNCOM005.T$QUAN$C > 0 THEN
            CISLI940.T$DOCN$L                               
       ELSE TDREC940.T$DOCN$L END                      NUM_NOTA,
       CASE WHEN ZNCOM005.T$QUAN$C > 0 THEN
            CISLI940.T$SERI$L                         
       ELSE TDREC940.T$SERI$L END                      SERIE,
...
where TCIBD001_G.T$NPCL$C = '1'     --ITEM GARANTIA ESTENDIDA

--SELECT * FROM BAANDB.TTCMCS060201  TCMCS060
		
--WHERE CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(ZNSLS400.T$DTEM$C, 
--         'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT') 
