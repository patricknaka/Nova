  SELECT /*+ use_concat no_cpu_costing */ 
        ...
        znfmd630_orig.t$cfrw$c                          TRANSPORTADORA, 
        znfmd630_orig.t$cono$c                          CONTRATO,
        ( select znfmd060.t$cdes$c
          from  baandb.tznfmd060301 znfmd060
          where znfmd060.t$cfrw$c = znfmd630_orig.t$cfrw$c 
            and znfmd060.t$cono$c = znfmd630_orig.t$cono$c )
                                                        NOME_CONTRATO,
        znfmd630_orig.t$fili$c                          FILIAL, 
        znfmd630_orig.t$vlfc$c                          FRETE_GTE,
        znfmd630_orig.t$vlft$c                          FRETE_TRANSPORTADORA,
        znfmd630_orig.t$orno$c                          ORDEM_VENDA, 
        znfmd630_orig.t$pecl$c                          NRO_ENTREGA,
        znfmd630_orig.t$cnfe$c                          NFE,
        znfmd630_orig.t$docn$c                          NUM_NFE,
        znfmd630_orig.t$seri$c                          SER_NFE,
        CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(znfmd630_orig.t$date$c,    
       'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')
        AT time zone 'America/Sao_Paulo') AS DATE)
                                                        DATA_EMISSAO,
        znfmd630_orig.t$ncte$c                          CTE,
        CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(tdrec940.t$idat$l,    
       'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')    
         AT time zone 'America/Sao_Paulo') AS DATE)    
                                                        EMISS_CTE,
        decode ( znfmd630_orig.t$stat$c,1, 'Aberto', 2, 'Fechado',znfmd630_orig.t$stat$c) 
                                                        STATUS,
        decode (znfmd630_orig.t$torg$c,1, 'Venda',2, 'Reversa', 7, 'Insucesso',znfmd630_orig.t$torg$c) 
                                                        TIPO_OR_FRE,
        ( select znfmd637.t$amnt$c
          from baandb.tznfmd637301 znfmd637
          where znfmd637.t$txre$c = znfmd630_orig.t$txre$c
            and znfmd637.t$line$c = znfmd630_orig.t$line$c
            and znfmd637.t$brty$c = 1 )   --ICMS
                                                        ICMS_FRETE,
        ( select znfmd637_PIS.t$amnt$c
          from    baandb.tznfmd637301 znfmd637_PIS
          where znfmd637_PIS.t$txre$c = znfmd630_orig.t$txre$c
            and znfmd637_PIS.t$line$c = znfmd630_orig.t$line$c
            and znfmd637_PIS.t$brty$c = 5  ) --PIS
                                                        PIS_FRETE,
        ( select znfmd637_COFINS.t$amnt$c 
          from baandb.tznfmd637301 znfmd637_COFINS
          where znfmd637_COFINS.t$txre$c = znfmd630_orig.t$txre$c
            and znfmd637_COFINS.t$line$c = znfmd630_orig.t$line$c
    ...
    (  select nvl(a.t$dsca,'Pedido Interno') t$dsca 
       ...
       where b.t$orno$c = znfmd630_orig.t$orno$c
  ...
  left join baandb.tcisli940301 cisli940
         on cisli940.t$fire$l = brnfe940.t$fire$l
         
  left join baandb.tznfmd630301 znfmd630_orig
         on znfmd630_orig.t$docn$c = cisli940.t$docn$l
        and znfmd630_orig.t$seri$c = cisli940.t$seri$l

WHERE znfmd630.t$ncte$c is null
  and tfacp200.t$tdoc = ' ' 
