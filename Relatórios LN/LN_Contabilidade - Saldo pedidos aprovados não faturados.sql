SELECT 
	...
	(ZNSLS401.VL_PRODUTO + ZNSLS401.VL_DISC + ZNSLS401.VL_FRET)		VL_TOTAL,
...
INNER JOIN (SELECT DISTINCT	A.T$NCIA$C, A.T$UNEG$C, A.T$PECL$C,
					A.T$SQPD$C, A.T$ENTR$C, A.T$ORNO$C
			FROM	BAANDB.TZNSLS004201	A
			)				ZNSLS004
	ON	ZNSLS004.T$ORNO$C	=	TDSLS400.T$ORNO

INNER JOIN (SELECT A.T$NCIA$C, A.T$UNEG$C, A.T$PECL$C, A.T$SQPD$C,
                    A.T$ENTR$C, MIN(A.T$DTAP$C) T$DTAP$C, MIN(A.T$DTEP$C) T$DTEP$C,
					SUM(A.T$VLUN$C*A.T$QTVE$C) VL_PRODUTO, 
					SUM(A.T$VLDI$C) VL_DISC, SUM(A.T$VLFR$C)	VL_FRET
			FROM	BAANDB.TZNSLS401201 A
			GROUP BY	A.T$NCIA$C, A.T$UNEG$C, A.T$PECL$C, A.T$SQPD$C, A.T$ENTR$C
			)				ZNSLS401
...
LEFT JOIN 	(SELECT A.T$NCIA$C, A.T$UNEG$C, A.T$PECL$C, A.T$SQPD$C, A.T$ENTR$C,	
...
INNER JOIN	BAANDB.TTCMCS013201					TCMCS013
...
WHERE	TDSLS400.T$HDST IN (10,20,25,30,45)
	...
	AND (	SELECT NVL(MAX(B.T$REFR$L), ' ')
			...
			INNER JOIN	BAANDB.TCISLI941201 B
