SELECT znsls401.t$orno$c, znsls401.t$pono$c,
  ...
  CAST((FROM_TZ(CAST(TO_CHAR(cisli940.t$date$l, 	'DD-MON-YYYY HH:MI:SS AM') AS 
                                                    TIMESTAMP), 'GMT') AT time zone sessiontimezone) AS DATE) 
                      DATA_NF_ENTRADA,
  CAST((FROM_TZ(CAST(TO_CHAR(znsls401.t$dtre$c, 	'DD-MON-YYYY HH:MI:SS AM') AS 
                                                    TIMESTAMP), 'GMT') AT time zone sessiontimezone) AS DATE)
                      COLETA_PREVISTA,
  CAST((FROM_TZ(CAST(TO_CHAR(znsls401.t$dtep$c, 	'DD-MON-YYYY HH:MI:SS AM') AS 
                                                    TIMESTAMP), 'GMT') AT time zone sessiontimezone) AS DATE)
                      RETORNO_PREVISTO,
  znsls401.t$itml$c	  NUME_ITEM,
...
WHERE 
  znsls400.t$ncia$c = znsls401.t$ncia$c 
AND 
  znsls400.t$uneg$c = znsls401.T$UNEG$c 
AND 
  znsls400.t$pecl$c = znsls401.T$pecl$c 
AND 
  znsls400.t$sqpd$c = znsls401.T$sqpd$c
AND 
  znsls401.t$entr$c = znfmd630.t$pecl$c
AND
  znsls401.t$orno$c = znfmd630.T$ORNO$C
AND
	cisli940.t$fdty$l = FGET.CODE_STAT
