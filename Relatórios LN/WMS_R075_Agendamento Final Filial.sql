                   ...
                   CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(RC.ADDDATE,    
                     'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')
                       AT time zone 'America/Sao_Paulo') AS DATE)
                                        DT_NR                
              from WMWHSE10.RECEIPT  RC
        inner join WMWHSE10.RECEIPTDETAIL RD
...
WHERE TDREC940.T$RFDT$L IN (1,2,5,10,28)
 ...
 Between :DataDe
     And :DataAte
   AND NVL(TDREC940.T$STAT$L, 0) IN (:Situacao)
...
"  AND NVL(TDREC940.T$STAT$L, 0) in (" + JOIN(Parameters!Situacao.Value, ", ") + ")  " &
"  " &
"ORDER BY 3,5,6   "
