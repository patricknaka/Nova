SELECT
  ...
  'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')
    AT time zone 'America/Sao_Paulo') AS DATE)        
                          DATA_EMISS,
  cisli940.t$stat$l       CODE_STATUS_NF_DEVOLUCAO, 
  DESC_DOMAIN_STAT.       DESCR DESCR_STATUS_NF_DEVOLUCAO, 
...
WHERE  tdrec941.t$rfdv$c != ' '
   ...
   AND cisli940.t$stat$l = (CASE WHEN :StatusNF = 0 THEN cisli940.t$stat$l ELSE :StatusNF END)
   AND Trim(tcibd001.t$citg) IN (:GrupoItem)
   AND ( (Trim(tdrec940.t$fovn$l) like '%' || Trim(:CNPJ) || '%') OR (:CNPJ is null) )