SELECT
  ...
  NVL(TCCOM125.T$CBAN, 'Não aplicável')     CODE_CONTA_PN_LOJISTA,
  
  CASE WHEN Trim(TFCMG011.T$BANK) is null and Trim(TFCMG011.T$DESC) is null
         THEN 'Não aplicável'
       ELSE TFCMG011.T$BANK || ' - ' ||
            TFCMG011.T$DESC
    END                                     FILIAL_BANCARIA,
  
  NVL(TCCOM125.T$BANO, 'Não aplicável')     CONTA_BANCARIA,
  NVL(TCCOM125.T$DACC$D, 'Não aplicável')   DIGITO_CONTA,
  ...
  ( select ABS(TFACP200T_AP.T$AMNT)
      from BAANDB.TTFACP200301 TFACP200T_AP
     where TFACP200T_AP.T$TTYP = TFACP200M.T$TTYP
       and TFACP200T_AP.T$NINV = TFACP200M.T$NINV
       and TFACP200T_AP.T$TPAY = 2
       and TFACP200T_AP.T$STEP = 16 )    VALOR_APAGAR_CAP
  
FROM       BAANDB.TTFACR200301 TFACR200T

INNER JOIN BAANDB.TTCCOM100301 TCCOM100_ACR 
        ON TCCOM100_ACR.T$BPID = TFACR200T.T$ITBP

INNER JOIN BAANDB.TTCCOM130301 TCCOM130_ACR 
        ON TCCOM130_ACR.T$CADR = TCCOM100_ACR.T$CADR

INNER JOIN BAANDB.TTFACR200301 TFACR200M  
        ON TFACR200M.T$TTYP = TFACR200T.T$TTYP
       AND TFACR200M.T$NINV = TFACR200T.T$NINV
       AND TFACR200M.T$TDOC = 'ENC'
   
INNER JOIN BAANDB.TTFACP200301 TFACP200M  
...
INNER JOIN ( select A.T$TTYP,
 ...
 LEFT JOIN BAANDB.TTFGLD008301 TFGLD008F  
        ON TFGLD008F.T$LEAC = TFGLD106.T$LEAC

 LEFT JOIN BAANDB.TTFGLD008301 TFGLD008FR
        ON TFGLD008FR.T$LEAC = TFGLD106R.T$LEAC
  
 LEFT JOIN BAANDB.TTFGLD008301 TFGLD008N  
        ON TFGLD008N.T$LEAC = TFGLD102.T$LEAC

 LEFT JOIN BAANDB.TTFGLD008301 TFGLD008NR
