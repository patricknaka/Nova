SELECT
		...
		ST_PAGMT.DSCA												ST_PGTO_TRANS_CAP,
		...
		TFCMG101.T$BASU												CODE_CONTA_PN_LOJISTA,
		TFCMG011.T$BANK || ' - '
		|| TFCMG011.T$DESC											FILIAL_BANCARIA,
		TFCMG001.T$BANO												CONTA_BANCARIA,
		TFCMG001.T$OFDG$L											DIGITO_CONTA,
...
FROM
			BAANDB.TTFACR200201	TFACR200T
INNER JOIN	BAANDB.TTCCOM100201	TCCOM100_ACR	ON	TCCOM100_ACR.T$BPID	=	TFACR200T.T$ITBP
INNER JOIN	BAANDB.TTCCOM130201	TCCOM130_ACR	ON	TCCOM130_ACR.T$CADR	=	TCCOM100_ACR.T$CADR
INNER JOIN	BAANDB.TTFACR200201	TFACR200M		ON	TFACR200M.T$TTYP	=	TFACR200T.T$TTYP
												AND	TFACR200M.T$NINV	=	TFACR200T.T$NINV
												AND	TFACR200M.T$TDOC	=	'ENC'
INNER JOIN	BAANDB.TTFACP200201	TFACP200M		ON	TFACP200M.T$TDOC	=	TFACR200M.T$TDOC
												AND	TFACP200M.T$DOCN	=	TFACR200M.T$DOCN
INNER JOIN	BAANDB.TTFACP200201 TFACP200T		ON	TFACP200T.T$TTYP	=	TFACP200M.T$TTYP
...
INNER JOIN (SELECT 	A.T$TTYP,
			...
			GROUP BY A.T$TTYP,
			         A.T$NINV) TFACP201		ON	TFACP201.T$TTYP		=	TFACP200T.T$TTYP
...
LEFT JOIN BAANDB.TTFCMG001201 TFCMG001		ON	TFCMG001.T$BANK		=	TFCMG101.T$BANK
...
LEFT JOIN BAANDB.TTFGLD008201 TFGLD008F		ON	TFGLD008F.T$LEAC	=	TFGLD106.T$LEAC
LEFT JOIN BAANDB.TTFGLD008201 TFGLD008N		ON	TFGLD008N.T$LEAC	=	TFGLD102.T$LEAC
...
--    AND TFACR200M.T$TTYP='RKL' AND TFACR200T.T$NINV=19