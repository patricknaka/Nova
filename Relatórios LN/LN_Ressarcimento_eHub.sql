SELECT
  ...
  case when trunc(znsls409.t$fdat$c) < TO_DATE('01-01-1980','DD-MM-YYYY') 
         then null
       else   CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(znsls409.t$fdat$c, 'DD-MON-YYYY HH24:MI:SS'), 
                'DD-MON-YYYY HH24:MI:SS'), 'GMT') AT time zone 'America/Sao_Paulo') AS DATE)   
  end                                                   DATA_FORCADO,
  CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(tdsls400dev.t$odat, 'DD-MON-YYYY HH24:MI:SS'), 
    'DD-MON-YYYY HH24:MI:SS'), 'GMT') AT time zone 'America/Sao_Paulo') AS DATE) 
                                                        DATA_OCORRENCIA,
  ...
  regexp_replace(tccom130_orig.t$fovn$l, '[^0-9]', '')  TRANSP_VENDA_CNPJ,    
  tcmcs080_orig.t$dsca                                  TRANSP_VENDA_NOME,
  regexp_replace(tccom130_dev.t$fovn$l, '[^0-9]', '')   TRANSP_COLETA_CNPJ,
  ...
  CASE WHEN cisli940orig.t$docn$l IS NULL 
         THEN SLI940_orig.t$docn$l 
       ELSE   cisli940orig.t$docn$l 
  END                                                   NF_ORIGINAL,
  CASE WHEN cisli940orig.t$docn$l IS NULL 
         THEN SLI940_orig.t$seri$l
       ELSE   cisli940orig.t$seri$l 
  END                                                   SERIE_ORIGINAL,
  ...
  CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(cisli940orig.t$date$l, 'DD-MON-YYYY HH24:MI:SS'), 
    'DD-MON-YYYY HH24:MI:SS'), 'GMT') AT time zone 'America/Sao_Paulo') AS DATE)          
                                                        DT_EMISSAO_NF_VENDA,    
  tdrec940rec.t$docn$l                                  NFE,
  tdrec940rec.t$seri$l                                  SERIE_NFE,
  CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(tdrec940rec.t$date$l, 'DD-MON-YYYY HH24:MI:SS'), 
    'DD-MON-YYYY HH24:MI:SS'), 'GMT') AT time zone 'America/Sao_Paulo') AS DATE)         
                                                        DATA_REFERENCIA_FISCAL,
  ...
  CASE WHEN tcmcs080_dev.t$cfrw IN ('T70', 'A46', 'A45', 'A44', 'T68', 'T73', 'T69', 'T71', 'T72', 'T31', 'T30') 
         THEN znfmd630dev.t$etiq$c 
       ELSE   NULL 
  END                                                   ETIQUETA,
  'Sim'                                                 FORCADA,
  ...
  CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(cisli940dev.t$date$l, 'DD-MON-YYYY HH24:MI:SS'), 
    'DD-MON-YYYY HH24:MI:SS'), 'GMT') AT time zone 'America/Sao_Paulo') AS DATE)
                                                        DT_NF_COLETA,
