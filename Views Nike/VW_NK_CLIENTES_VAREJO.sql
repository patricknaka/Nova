...
SELECT DISTINCT
  ...
  NVL(SUBSTR(replace(replace(replace(
     case when length(tccom130.t$telp) < 8 then case when length(ZNSLS400.t$te1f$c) < 8 then ZNSLS400.t$te2f$c else ZNSLS400.t$te1f$c END ELSE tccom130.t$telp END
     ,'(',''),')',''),'-',''),3,13),' ') TELEFONE,   ---SUBSTR(tccom130.t$telp,3,13)   TELEFONE,           --17
  tccom130.t$tefx               FAX,                --18
  ' '                           LIMITE_CREDITO,     --19
  ' '                           SEM_CREDITO,        --20
--  ZNSLS400.t$dtna$c            ANIVERSARIO,        --21
  CAST(case when ZNSLS400.t$dtna$c is null then '' else cast(ZNSLS400.t$dtna$c as varchar(20)) end AS VARCHAR(20))           ANIVERSARIO,        --21
  ...
  ' '                           OBS,                --25
    ...
    CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(SLI940.t$sadt$l, 'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')
          AT time zone 'America/Sao_Paulo') AS DATE)
                                DT_ULT_UPDATE
...
FROM  baandb.ttccom100601 tccom100
  ...
  LEFT JOIN baandb.ttccom130601 tccom130
         ON tccom130.t$cadr = SLI940.end_entrega
...
--  ZNSLS400.T$dtna$c            ANIVERSARIO,        --21
  ...
  ' '                           OBS,                --25
    ...
    CAST((FROM_TZ(TO_TIMESTAMP(TO_CHAR(REC940.t$adat$l, 'DD-MON-YYYY HH24:MI:SS'), 'DD-MON-YYYY HH24:MI:SS'), 'GMT')
          AT time zone 'America/Sao_Paulo') AS DATE)
                            DT_ULT_UPDATE
