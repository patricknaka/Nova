CREATE OR REPLACE VIEW VW_NK_LOJA_ENT_SAI_PRODUTO AS
SELECT
--***************************************************************************************************************************
--        SAIDA
--***************************************************************************************************************************
...
SELECT
    ...
    CASE WHEN WHINH301.t$item IS NULL THEN
        TCIBD001_2.T$ITEM ELSE WHINH301.T$ITEM END PRODUTO,
    '01'                                          COR_PRODUTO,
    ...
    CASE WHEN WHINH301.T$RQUA IS NULL THEN
        CAST((TDREC941.T$QNTY$L * TDREC941.T$PRIC$L) AS NUMERIC(38,4))
    ELSE
        CAST((WHINH301.T$RQUA * TDREC941.T$PRIC$L) AS NUMERIC(38,4)) END  VALOR,
    CAST(TDREC941.T$PRIC$L AS NUMERIC(38,4)  )     PRECO1,

    CASE WHEN WHINH301.t$rqua IS NULL THEN
          TDREC941.T$QNTY$L
    ELSE  WHINH301.T$RQUA END                     QTDE_ITEM,
...
LEFT JOIN  BAANDB.TTCIBD001601  TCIBD001_1
       ON  TCIBD001_1.T$ITEM = WHINH301.T$ITEM    --usado para Item minucioso
...
LEFT JOIN  BAANDB.TTCIBD001601  TCIBD001_2
       ON  TCIBD001_2.T$ITEM = tdrec941.T$ITEM$L
...
WHERE TDREC940.T$STAT$L IN (4,5,6)  --4-Aprovado, 5-Aprovado com Problemas, 6-estornada
  ...
  AND (WHINH301.T$RQUA != 0 OR WHINH301.T$RQUA IS NULL)
  AND tdrec940.t$doty$l != 8    --8-conhecimento de frete
  AND tdrec940.t$rfdt$l != 14    --Nota de Débito-14

--  and tdrec941.t$fire$l in ('000010510','000010511')
